...
int main()
{
    ...
    MinCostMaximumFlow MCMF(n, m);

    for (int person = 1; person <= n; person++)
    {
        int c;
        cin >> c;
        MCMF.add_edge(MCMF.source, person, c, 0);
    }

    for (int bookstore = n + 1; bookstore <= n + m; bookstore++)
    {
        int c;
        cin >> c;
        MCMF.add_edge(bookstore, MCMF.sink, c, 0);
    }
    ...
    for (int bookstore = n + 1; bookstore <= n + m; bookstore++)
    {
        for (int person = 1; person <= n; person++)
        {
            int cost;
            cin >> cost;
            MCMF.add_edge(person, bookstore, INF, cost);
        }
    }
