...
int main()
{
    ...
    while (T--)
    {
        ...
        for (int i = 0; i < n; i++)
        {
            int a, b, c, d, e;
            cin >> a >> b >> c >> d >> e;
            garo.push_back({min(a, c), b, max(a, c), d, e});
        ...
        for (int i = 0; i < m; i++)
        {
            int a, b, c, d, e;
            cin >> a >> b >> c >> d >> e;
            sero.push_back({a, min(b, d), c, max(b, d), e});
            MCMF.add_edge(n + i + 1, MCMF.sink, 1, 0);
        }
        ...
        for (int i = 0; i < n; i++)
        {
            for (int j = 0; j < m; j++)
            {
                auto [x1, y1, x2, y2, w1] = garo[i];
                auto [x3, y3, x4, y4, w2] = sero[j];

                if (x1 <= x3 && x3 <= x2 && y3 <= y1 && y1 <= y4)
                    MCMF.add_edge(i + 1, n + j + 1, 1, -w1 * w2);
            }
        }
