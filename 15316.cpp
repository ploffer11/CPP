...
main()
{
    ...
    for (int i = 1; i <= m; i++)
    {
        cin >> state[i];
        if (state[i])
            query.push_back({ADD, edge[i].first, edge[i].second});
    }
    ...
    while (q--)
    {
        ...
        {
            cin >> y >> z;
            query.push_back({QUERY, y, z});
        }
        else 
        {
            cin >> y;
            query.push_back({(state[y] ? DELETE : ADD), edge[y].first, edge[y].second});
            state[y] ^= 1;
        }
    }
    
    OfflineDynamicConnectivity ODC(n, query.size());
